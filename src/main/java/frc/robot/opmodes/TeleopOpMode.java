...
public class TeleopOpMode extends PeriodicOpMode {
  ...
  public TeleopOpMode(Robot robot) {
    ...
    // stick there forever. .until(arm::isAtTarget) gives the hold an ending right here - don't go
    // add a special "AndWait" method to the subsystem. In an auto, also add .withTimeout(seconds)
    // as a seatbelt: if the arm never quite arrives, the routine moves on instead of getting
